...
export default function Index() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const response = await RequestService.getParam("/orders-service-items", {
        page: currentPage,
        per_page: perPage,
      });
  ...
  return (
    <main className="h-full grow flex flex-col border border-[#E7E7E7] overflow-y-auto">
      <TabHead name="Orders Service Items"></TabHead>
      <div className="p-5 w-full flex flex-col gap-3">
        <div className="w-full flex gap-2 items-center text-sm">
          <Search
            ...
            setValue={setSearch}
            width="250px"
            placeholder="Search orders service items..."
          />
          <div className="flex items-center gap-2 ml-auto">
            <Button
              name="Add Order Service Item"
              onClick={() => {
                setShowCreate(true);
